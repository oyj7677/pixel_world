...
async function createRoom(page: Page, roomName = uniqueRoomName('E2E 친구 방'), ownerName = '방장 민아') {
  ...
  const roomPath = (await roomLink.getAttribute('href')) ?? '';
...
async function waitForRoomCanvas(page: Page, roomPath: string) {
  ...
  await expect(page.getByRole('grid', { name: '32×32 픽셀 캔버스', exact: true })).toBeVisible();
}
...
test('creator creates room and invitee leaves named Quick Pixel', async ({ browser }) => {
  ...
  try {
    ...
    await expect(creator.getByRole('button', { name: '픽셀 16,16', exact: true })).toHaveCSS(
      'background-color',
      quickPixelColor
    );
  } finally {
...
test('room members can copy a fresh invite address from the room screen', async ({ browser }) => {
  const creator = await browser.newPage();
  const { context: inviteeContext, page: invitee } = await newIsolatedPage(browser);
...
test('two rooms do not receive each other realtime pixel updates', async ({ browser }) => {
  ...
  try {
    ...
    const roomAPixel = roomAPage.getByRole('button', { name: '픽셀 16,16', exact: true });
    const roomBPixel = roomBPage.getByRole('button', { name: '픽셀 16,16', exact: true });
...
test('invalid invite cannot place a pixel', async ({ page, request }) => {
  ...
  const roomPublicId = roomPath.replace('/r/', '');
  ...
  await expect(response.json()).resolves.toMatchObject({
    ...
    message: 'Use a fresh invite link to place your first Quick Pixel.'
  });
});
